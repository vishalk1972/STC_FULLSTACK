...
const AdminGetStudent = () => {
  ...
  const handleDelete=(id)=>{
    if(admin && window.confirm("Are Your Sure ! You Want To Delete This Data"))
      {
        axios.delete(`${backurl}/api/adminDashboard/deleteStudent/${id}`,
        {
            headers:{
                Authorization: `Bearer ${admin.token}`
            }
  ...
  return (
    ...
    <div className='bg-[#d5e0fb] min-h-screen  '>
       ...
       <div className='flex justify-end md:gap-5 gap-2 pt-5 md:px-5 px-2'>
          <Link to="/admin/dashboard/addStudent"><button className='bg-[#041643] text-white  py-1 hover:bg-slate-600 md:px-4 px-2 md:text-xl text-lg rounded-xl focus:outline-none focus:shadow-outline'>Add Student</button></Link>
        ...
        <h1 className='md:text-3xl text-3xl my-2 md:flex justify-center font-bold md:mb-5 mb-2 mx-2'>List Of All Students</h1> 
